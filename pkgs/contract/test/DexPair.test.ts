...
describe("DexPair", () => {
  ...
  describe("デプロイメントと初期化", () => {
    it("ペアが正しく初期化されている", async () => {
      ...
      expect([token0, token1].sort()).to.deep.equal([tokenAAddress, tokenBAddress].sort());
    });
  });
  ...
  describe("流動性提供 (Mint)", () => {
    it("初回流動性提供が正常に動作する", async () => {
      ...
      const expectedLiquidity = ethers.parseEther("10") - BigInt(MINIMUM_LIQUIDITY);
    ...
    it("追加流動性提供が正常に動作する", async () => {
      ...
      await tokenA.connect(user1).transfer(await pair.getAddress(), initialAmount0);
      await tokenB.connect(user1).transfer(await pair.getAddress(), initialAmount1);
      ...
      await tokenA.connect(user2).transfer(await pair.getAddress(), additionalAmount0);
      await tokenB.connect(user2).transfer(await pair.getAddress(), additionalAmount1);
      ...
      expect(await pair.balanceOf(user2.address)).to.equal(expectedAdditionalLiquidity);
    });

    it("不均等な流動性提供は適切に処理される", async () => {
      // 初回流動性提供
      await tokenA.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("10"));
      await tokenB.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("20"));
      await pair.mint(user1.address);
      
      // 不均等な追加流動性提供
      await tokenA.connect(user2).transfer(await pair.getAddress(), ethers.parseEther("5"));
      await tokenB.connect(user2).transfer(await pair.getAddress(), ethers.parseEther("15"));
    ...
    it("流動性が不足している場合は失敗する", async () => {
      await expect(pair.mint(user1.address))
        .to.be.revertedWithPanic(0x11); // オーバーフローパニック
    });
  });
  ...
  describe("流動性削除 (Burn)", () => {
    beforeEach(async () => {
      // 初期流動性を提供
      await tokenA.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("20"));
      await tokenB.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("20"));
      await pair.mint(user1.address);
    });
    ...
    it("無効な受取人アドレスでは失敗する", async () => {
      ...
      await expect(pair.burn(await pair.getAddress()))
  ...
  describe("スワップ", () => {
    ...
    it("Token0からToken1へのスワップが正常に動作する", async () => {
      ...
      if (isTokenAToken0) {
        ...
        await tokenB.connect(user2).transfer(await pair.getAddress(), swapAmount);
      }
      
      // リザーブを取得してスワップ量を計算
      const reserves = await pair.getReserves();
      const reserveIn = isTokenAToken0 ? reserves._reserve0 : reserves._reserve1;
      const reserveOut = isTokenAToken0 ? reserves._reserve1 : reserves._reserve0;
    ...
    it("不正な出力量では失敗する", async () => {
      await expect(pair.swap(0, 0, user2.address, "0x"))
    ...
    it("リザーブを超える出力量では失敗する", async () => {
      ...
      await expect(pair.swap(excessiveAmount, 0, user2.address, "0x"))
    ...
    it("無効な受取人アドレスでは失敗する", async () => {
      ...
      await expect(pair.swap(0, ethers.parseEther("0.99"), tokenAAddress, "0x"))
        .to.be.revertedWithCustomError(pair, "InvalidTo");
    });
  });
  ...
  describe("リザーブ同期", () => {
    it("sync関数が正常に動作する", async () => {
      // 流動性を提供
      await tokenA.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("10"));
      await tokenB.connect(user1).transfer(await pair.getAddress(), ethers.parseEther("10"));
      await pair.mint(user1.address);
