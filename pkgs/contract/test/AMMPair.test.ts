...
describe("AMMPair", () => {
  ...
  describe("Liquidity Addition (mint)", () => {
    it("Should mint liquidity tokens for first liquidity provider", async () => {
      ...
      expect(await pair.balanceOf(owner.address)).to.be.closeTo(expectedLiquidity, ethers.parseEther("0.001"));
      expect(await pair.totalSupply()).to.be.closeTo(ethers.parseEther("141.421356237309504880"), ethers.parseEther("0.001"));
    ...
    it("Should mint liquidity tokens proportionally for subsequent providers", async () => {
      ...
      expect(await pair.balanceOf(addr1.address)).to.be.closeTo(expectedLiquidity, ethers.parseEther("0.001"));
    });
  ...
  describe("Liquidity Removal (burn)", () => {
    ...
    it("Should fail when burning with insufficient liquidity", async () => {
      await expect(pair.burn(owner.address)).to.be.revertedWith("AMMPair: INSUFFICIENT_LIQUIDITY_BURNED");
    });
  ...
  describe("Token Swapping", () => {
    ...
    it("Should fail when output amount is zero", async () => {
      ...
      ).to.be.revertedWith("AMMPair: INSUFFICIENT_OUTPUT_AMOUNT");
    });
    ...
    it("Should fail when output exceeds reserves", async () => {
      ...
      await expect(
        pair.swap(reserve0, 0, addr1.address, "0x")
      ).to.be.revertedWith("AMMPair: INSUFFICIENT_LIQUIDITY");
      
      await expect(
        pair.swap(0, reserve1, addr1.address, "0x")
      ).to.be.revertedWith("AMMPair: INSUFFICIENT_LIQUIDITY");
    });

    it("Should fail when swapping to token addresses", async () => {
      await expect(
        pair.swap(1, 0, await tokenA.getAddress(), "0x")
      ).to.be.revertedWith("AMMPair: INVALID_TO");
      
      await expect(
        pair.swap(0, 1, await tokenB.getAddress(), "0x")
      ).to.be.revertedWith("AMMPair: INVALID_TO");
    });

    it("Should fail when swapping to zero address", async () => {
      await expect(
        pair.swap(1, 0, ethers.ZeroAddress, "0x")
      ).to.be.revertedWith("AMMPair: INVALID_TO");
    });
  });
  ...
  describe("AMM Formula Verification (x * y = k)", () => {
    ...
    it("Should maintain K value during liquidity operations", async () => {
      ...
      const expectedK = kBefore * (reserve0After * reserve0After) / (reserve0Before * reserve0Before);
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("Should handle zero liquidity edge cases", async () => {
      // 流動性がゼロの状態でのスワップ試行
      await expect(
        pair.swap(1, 0, addr1.address, "0x")
      ).to.be.revertedWith("AMMPair: INSUFFICIENT_LIQUIDITY");
    });
