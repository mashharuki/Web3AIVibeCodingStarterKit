...
describe("DEX Integration Tests", () => {
  ...
  describe("完全なDEXワークフロー", () => {
    ...
    it("流動性提供からスワップまでの完全なフローが動作する", async () => {
      ...
      // ルーターがトークンを使用できるよう承認
      await tokenA
        .connect(liquidity_provider)
        .approve(await router.getAddress(), liquidityAmountA);
      await tokenB
        .connect(liquidity_provider)
        .approve(await router.getAddress(), liquidityAmountB);
      ...
      await tokenA
        .connect(trader1)
        .approve(await router.getAddress(), swapAmountA);
      ...
      await tokenB
        .connect(trader2)
        .approve(await router.getAddress(), swapAmountB);
      ...
      const providerBalanceABefore = await tokenA.balanceOf(
        liquidity_provider.address
      );
      const providerBalanceBBefore = await tokenB.balanceOf(
        liquidity_provider.address
      );
      ...
      const providerBalanceAAfter = await tokenA.balanceOf(
        liquidity_provider.address
      );
      const providerBalanceBAfter = await tokenB.balanceOf(
        liquidity_provider.address
      );
    ...
    it("流動性が不足している状況での大量スワップは失敗する", async () => {
      ...
      await tokenA
        .connect(trader1)
        .approve(await router.getAddress(), excessiveSwapAmount);
  ...
  describe("エッジケース", () => {
    ...
    it("最小流動性の処理が正しく動作する", async () => {
      ...
      await tokenA
        .connect(liquidity_provider)
        .approve(await router.getAddress(), minAmountA);
      await tokenB
        .connect(liquidity_provider)
        .approve(await router.getAddress(), minAmountB);
