 ...
 */
task("dexInfo", "デプロイされたDEXコントラクトの情報を表示します")
  .setAction(async (_taskArgs: any, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      const dexFactory = await hre.ethers.getContractAt("DexFactory", contracts.dex.DexFactory) as any;
      const dexRouter = await hre.ethers.getContractAt("DexRouter", contracts.dex.DexRouter) as any;
      ...
      console.log(`   Total Supply: ${hre.ethers.formatEther(await tokenA.totalSupply())} TKA`);
      console.log();
      ...
      console.log(`   Total Supply: ${hre.ethers.formatEther(await tokenB.totalSupply())} TKB`);
      console.log();
      ...
      console.log(`   Total Supply: ${hre.ethers.formatEther(await pair.totalSupply())}`);
      console.log();
      ...
      console.error("❌ コントラクト情報の取得中にエラーが発生しました:", error);
      throw error;
    }
  });
