 ...
 */
task("addLiquidityOptimal", "現在のプール比率に基づいて最適な流動性追加を行います")
  .addParam("amount", "基準となる量（Ether単位）")
  .addParam("token", "基準にするトークン（A または B）", "A")
  .addParam("slippage", "スリッページ（パーセント）", "5")
  .setAction(async (taskArgs: { amount: string; token: string; slippage: string }, hre: HardhatRuntimeEnvironment) => {
    const network = hre.network.name;
    const { amount, token, slippage } = taskArgs;
    
    console.log(`💧 ${network} ネットワークで最適な流動性追加を実行します...\n`);
    ...
    try {
      ...
      console.log(`   Ratio A/B: ${Number(hre.ethers.formatEther(reserveA)) / Number(hre.ethers.formatEther(reserveB))}`);
      console.log();
      ...
      if (token.toUpperCase() === "A") {
        ...
        console.log(`💡 Using ${amount} TKA as base, optimal TKB: ${hre.ethers.formatEther(amountBDesired)}`);
      } else {
        // TokenBを基準にTokenAの最適量を計算
        amountBDesired = baseAmount;
        amountADesired = (baseAmount * reserveA) / reserveB;
        console.log(`💡 Using ${amount} TKB as base, optimal TKA: ${hre.ethers.formatEther(amountADesired)}`);
      }
      ...
      if (balanceA < amountADesired) {
        throw new Error(`TokenAの残高が不足しています。必要: ${hre.ethers.formatEther(amountADesired)} TKA, 現在: ${hre.ethers.formatEther(balanceA)} TKA`);
      }
      if (balanceB < amountBDesired) {
        throw new Error(`TokenBの残高が不足しています。必要: ${hre.ethers.formatEther(amountBDesired)} TKB, 現在: ${hre.ethers.formatEther(balanceB)} TKB`);
      }
      ...
      console.log(`🔸 TokenA Desired: ${hre.ethers.formatEther(amountADesired)} TKA`);
      console.log(`🔹 TokenB Desired: ${hre.ethers.formatEther(amountBDesired)} TKB`);
      ...
      if (receipt && receipt.status === 1) {
        ...
        console.log(`📤 使用したTokenA: ${hre.ethers.formatEther(usedA)} TKA`);
        console.log(`📤 使用したTokenB: ${hre.ethers.formatEther(usedB)} TKB`);
      ...
      console.error("❌ 流動性追加中にエラーが発生しました:", error.message || error);
      throw error;
    }
  });
