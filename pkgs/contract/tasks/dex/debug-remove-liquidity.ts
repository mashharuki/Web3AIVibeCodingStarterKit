...
task("debugRemoveLiquidity", "流動性削除のデバッグを行います")
  ...
  .setAction(async (taskArgs: { liquidity: string }, hre: HardhatRuntimeEnvironment) => {
    const network = hre.network.name;
    const { liquidity } = taskArgs;
    
    console.log(`🔍 ${network} ネットワークで流動性削除をデバッグします...\n`);
    ...
    try {
      ...
      console.log(`   User LP Balance: ${hre.ethers.formatEther(lpBalance)} LP`);
      console.log(`   Total LP Supply: ${hre.ethers.formatEther(totalSupply)} LP`);
      console.log(`   Removing Amount: ${liquidity} LP`);
      console.log(`   LP Percentage: ${(Number(liquidity) / Number(hre.ethers.formatEther(totalSupply)) * 100).toFixed(4)}%`);
      console.log();
      ...
      if (lpBalance < liquidityAmount) {
        ...
        console.log(`   不足: ${hre.ethers.formatEther(liquidityAmount - lpBalance)} LP`);
        return;
      }
      ...
      console.log(`   TokenA (TKA): ${hre.ethers.formatEther(expectedTokenA)}`);
      console.log(`   TokenB (TKB): ${hre.ethers.formatEther(expectedTokenB)}`);
      console.log();
      ...
      console.log(`   Sufficient: ${allowance >= liquidityAmount ? "✅ Yes" : "❌ No"}`);
      console.log();
      ...
      try {
        ...
        console.log(`   Will receive TokenA: ${hre.ethers.formatEther(result[0])} TKA`);
        console.log(`   Will receive TokenB: ${hre.ethers.formatEther(result[1])} TKB`);
        ...
        if (simError.message.includes("INSUFFICIENT_LIQUIDITY")) {
          ...
          console.log("🔍 Analysis: Router への LP Token 承認が不足しています");
        } else {
          console.log("🔍 Analysis: 不明なエラーです。コントラクトログを確認してください");
          
          // より詳細なデバッグ情報
          console.log(`\n🔍 Additional Debug Information:`);
          console.log(`   LP Balance >= Liquidity: ${lpBalance >= liquidityAmount}`);
          console.log(`   Allowance >= Liquidity: ${allowance >= liquidityAmount}`);
