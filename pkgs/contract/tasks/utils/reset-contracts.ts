 ...
 */
task("resetContractAddressesJson", "コントラクトアドレスJSONファイルをリセットします").setAction(
  async (_taskArgs: any, hre: HardhatRuntimeEnvironment) => {
    const network = hre.network.name;
    
    console.log(`🗑️ ${network} ネットワークのコントラクトアドレスファイルをリセットします...\n`);

    try {
      // コントラクトアドレスJSONファイルをリセット
      resetContractAddressesJson({ network });
      
      console.log(`✅ ${network} のコントラクトアドレスファイルをリセットしました`);
