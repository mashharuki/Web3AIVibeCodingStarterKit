...
task("mintTokens", "TokenAまたはTokenBをmintします")
  ...
  .setAction(async (taskArgs: { token: string; amount: string; to?: string }, hre: HardhatRuntimeEnvironment) => {
    const network = hre.network.name;
    const { token, amount, to } = taskArgs;
    
    console.log(`🪙 ${network} ネットワークで ${token} を ${amount} mint します...\n`);
    ...
    try {
      ...
      if (!contracts || !contracts.tokens) {
        console.error(`❌ ${network} ネットワークのトークンアドレスが見つかりません`);
        return;
      }
      ...
      if (token === "TokenA") {
        ...
        tokenContract = await hre.ethers.getContractAt("TokenA", tokenAddress) as any;
        tokenSymbol = "TKA";
        ...
        tokenContract = await hre.ethers.getContractAt("TokenB", tokenAddress) as any;
        tokenSymbol = "TKB";
      } else {
      ...
      console.log(`📊 mint前の残高: ${hre.ethers.formatEther(balanceBefore)} ${tokenSymbol}`);
      ...
      console.log(`📊 mint後の残高: ${hre.ethers.formatEther(balanceAfter)} ${tokenSymbol}`);
      console.log(`📈 増加量: ${hre.ethers.formatEther(balanceAfter - balanceBefore)} ${tokenSymbol}`);

      console.log("\n🎯 次のステップ:");
      console.log("• npx hardhat checkBalances --address YOUR_ADDRESS (残高確認)");
      console.log("• npx hardhat addLiquidity --amount-a 100 --amount-b 100 (流動性追加)");
...
task("checkBalances", "指定したアドレスのToken残高を確認します")
  .addOptionalParam("address", "残高を確認するアドレス（省略時は実行者のアドレス）")
