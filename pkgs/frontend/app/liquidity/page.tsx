                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="text-sm text-gray-300 bg-black/20 px-3 py-1 rounded-lg">
                          💰 残高: {tokenABalance.data ? Number(formatEther(tokenABalance.data)).toLocaleString() : "0"} {TOKEN_INFO[tokenA].symbol}
                        </div>
                        {baseToken === "A" && (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="text-sm text-gray-300 bg-black/20 px-3 py-1 rounded-lg">
                          💰 残高: {tokenBBalance.data ? Number(formatEther(tokenBBalance.data)).toLocaleString() : "0"} {TOKEN_INFO[tokenB].symbol}
                        </div>
                        {baseToken === "B" && (
                  ...
                  <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-400/20 p-6 backdrop-blur-sm">
                    ...
                    <div className="relative flex items-center space-x-4">
                      ...
                      <div className="flex-1">
                        <h4 className="text-lg font-bold text-yellow-300 mb-1">最適量を計算中...</h4>
                        <p className="text-yellow-200 text-sm">プールの現在比率を取得して最適な流動性量を算出しています</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {hasError && error && (
                  <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-400/20 p-6 backdrop-blur-sm">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1">
                        <h4 className="text-lg font-bold text-red-300 mb-1">計算エラー</h4>
                        <p className="text-red-200 text-sm">{error.message || "計算に失敗しました"}</p>
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    <div className="relative">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                          <div className="text-xs font-semibold text-gray-300 uppercase tracking-wide mb-1">現在のリザーブ</div>
                          <div className="text-sm font-bold text-white">
                            {reserveA.toFixed(4)} TKA / {reserveB.toFixed(4)} TKB
                          </div>
                        </div>
                        
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                          <div className="text-xs font-semibold text-gray-300 uppercase tracking-wide mb-1">プール比率</div>
                          <div className="text-sm font-bold text-white">
                        ...
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                          <div className="text-xs font-semibold text-gray-300 uppercase tracking-wide mb-1">計算基準</div>
                          <div className="text-sm font-bold text-white">
                            Token{baseToken} ベース
                          </div>
                        </div>
                      ...
                      <div className="bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-xl p-4 border border-emerald-400/30">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-semibold text-emerald-200">最適流動性量</span>
                        </div>
                        <div className="text-lg font-bold text-white">
                          {Number(optimalAmountA).toFixed(6)} TKA / {Number(optimalAmountB).toFixed(6)} TKB
                        </div>
                        <div className="text-xs text-emerald-300 mt-1">
                          💡 {baseToken === "A" ? "TokenA" : "TokenB"}を基準として自動計算されました
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <TabsContent value="remove" className="space-y-6">
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <Label className="text-base font-semibold text-gray-800">削除するLPトークン量</Label>
                  <div className="relative">
                    <Input
                      type="number"
                      placeholder="0.0"
                  ...
                  <div className="text-center">
                    <p className="text-sm text-gray-600 bg-gray-100 px-3 py-2 rounded-lg inline-block">
                      💰 LP残高: {lpTokenBalance.data ? formatEther(lpTokenBalance.data) : "0"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="bg-gradient-to-br from-red-50 to-pink-50 p-6 rounded-2xl border border-red-100">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white/70 p-4 rounded-xl">
                        <div className="text-xs text-gray-500 mb-1">{TOKEN_INFO[tokenA].symbol}</div>
                        <div className="text-lg font-bold text-gray-800">
                          {Number(formatEther(pairInfo.reserves[0])).toLocaleString()}
                        </div>
                      </div>
                      <div className="bg-white/70 p-4 rounded-xl">
                        <div className="text-xs text-gray-500 mb-1">{TOKEN_INFO[tokenB].symbol}</div>
                        <div className="text-lg font-bold text-gray-800">
                          {Number(formatEther(pairInfo.reserves[1])).toLocaleString()}
                        </div>
                      </div>
                    </div>
                  </div>
